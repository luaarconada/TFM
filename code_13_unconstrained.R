...
for (data in data_list){
  ...
  #for (fold in seq(0, num_folds - 1)) {
    ...
    # read results
    POST_results <- read.csv(file.path(res_path, 'in2post1', paste0(data, '_', fold, '_AD_POST_training_results_dtest.csv')))
    ...
    write.csv(test_results, file.path(res_path, 'in2post1unfinal', paste0( data, '_', fold, '_AD_bench_maxprof_results.csv')), row.names = T)
 # }
}
